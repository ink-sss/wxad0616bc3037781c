...
const loginPageSourcePath = join(root, "src/pages/login/login.vue");
...
async function loadLoginPageToolsModule() {
  ...
  const sourcePath = join(root, "src/pages/login/page-tools.js");
...
test("developer tools login reuses the local session without calling login APIs", async () => {
  ...
  assert.equal(session.token, "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..fAC4f3PLC9RmmZSroauQ3V9Wfjf3HmTlrSVJcM6QG_Y");
  assert.equal(session.user_id, 874);
  assert.equal(session.open_id, "ot-Fn3Z4r2e0iN6UpuTl1MGUysg8");
  assert.equal(session.im_user_id, "customer_874");
  assert.equal(session.im_user_sig, "YWMtC7JZFGFTEfGl5qtxAIs4lxDcftnmIkpPnhC-J8NjAz5dDO8QYVER8b-J5xmES75EAwMAAAGemtqGyQABUYC25uR8ZjJfZn7jKdGZSv4KAqure6IJNcH7iqREAYhGPg");
  ...
  assert.equal(storage.get("user_id"), 874);
...
test("login page does not depend on the wechat-login plugin component", async () => {
  ...
  const loginPage = pagesConfig.pages.find((page) => page.path === "pages/login/login");

  assert.ok(loginPage);
  assert.equal(loginPageSource.includes("<wechat-login"), false);
  assert.equal(JSON.stringify(loginPage.style || {}).includes("plugin://wx43134e071b752953/wechat-login"), false);
});
