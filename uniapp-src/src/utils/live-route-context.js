...
export function buildBroadcastReturnPath(input = {}) {
  const params = [];
  ...
  const isReplay =
    input.isReplay === true ||
    replay === "1" ||
    mode.toLowerCase() === "replay" ||
    liveType.toLowerCase() === "replay";

  appendParam(params, "roomCode", roomCode);
  appendParam(params, "liveId", liveId);
  if (roomId && roomId !== liveId) appendParam(params, "roomId", roomId);
  appendParam(params, "tenantId", tenantId);
  if (shareCode && shareCode !== roomCode) appendParam(params, "shareCode", shareCode);
