...
export default {
  ...
  onShareAppMessage() {
    return {
      ...
      path: '/pagesPlus/main/diy-page/diy-page?' + this.shareParams({ page_id: this.page_id })
    }
  },
  methods: {
    shareParams(extra = {}) {
    ...
    async scanQrcode() {
      try {
        ...
        const url = '/pagesPlus/main/store/clerkorder?order_no=' + encodeURIComponent(result.result || '')
