...
function requestByContext(vm, options) {
  ...
  return common_vendor.index.request({
    ...
    header: options.method === "POST" ? { "content-type": "application/json;charset=UTF-8" } : void 0,
    success(response) {
      if (response.statusCode !== 200 || !isObject(response.data))
        return false;
...
function requestFun(app) {
  ...
  app.config.globalProperties.doLogin = function doLogin() {
    ...
    services_h5AuthContext.redirectToNativeLogin();
  };
}
