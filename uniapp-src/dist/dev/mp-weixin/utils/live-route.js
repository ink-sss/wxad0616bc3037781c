...
function normalizeLiveRouteOptions(query = {}) {
...
function buildBroadcastEntryUrl(input = {}) {
  const options = normalizeLiveRouteOptions(input);
  const params = [];
  if (options.roomCode)
    params.push(`roomCode=${encodeURIComponent(options.roomCode)}`);
  if (options.liveId)
    params.push(`liveId=${encodeURIComponent(options.liveId)}`);
  if (options.roomId && options.roomId !== options.liveId)
  ...
  if (options.orientation)
    params.push(`orientation=${encodeURIComponent(options.orientation)}`);
  if (options.videoId)
    params.push(`videoId=${encodeURIComponent(options.videoId)}`);
  if (options.termId)
    params.push(`termId=${encodeURIComponent(options.termId)}`);
  return `/pages/broadcast/entry${params.length ? `?${params.join("&")}` : ""}`;
}
function isReplayEntry(options = {}, detail = {}) {
  if (String(options.replay || "") === "1")
    return true;
  if (String(options.mode || "") === "replay")
    return true;
  if (Number(detail.roomGroupType || detail.groupType || 0) === 1)
    return true;
  const statusText = String(detail.liveStatusText || detail.statusText || detail.live_status_text || "").trim();
  if (/回放|录播/.test(statusText))
    return true;
...
function classifyLiveSource(url = "") {
  ...
  if (!value)
    return { type: "", component: "", rank: 999 };
  if (value.startsWith("rtmp://"))
    return { type: "rtmp", component: "live-player", rank: 10 };
...
function getMiniProgramLiveCandidates(detail = {}, streamInfo = {}) {
  const candidates = [];
...
function getBestLiveUrl(detail = {}, options = {}) {
  ...
  const match = preferredComponent ? candidates.find((candidate) => candidate.component === preferredComponent) : candidates[0];
  return (match || candidates[0] || {}).url || "";
}
function getBestReplayUrl(detail = {}, replayVideo = {}) {
  var _a, _b;
  return replayVideo.videoUrl || replayVideo.video_url || replayVideo.url || replayVideo.playUrl || replayVideo.play_url || replayVideo.replayUrl || replayVideo.replay_url || replayVideo.hlsUrl || replayVideo.hls_url || replayVideo.m3u8Url || replayVideo.m3u8_url || ((_a = detail.replayVideo) == null ? void 0 : _a.videoUrl) || ((_b = detail.replayVideo) == null ? void 0 : _b.video_url) || detail.videoUrl || detail.video_url || detail.pullHlsUrl || detail.pull_hls_url || detail.hlsUrl || detail.hls_url || detail.m3u8Url || detail.m3u8_url || detail.pullUrl || detail.pull_url || detail.playUrl || detail.play_url || "";
}
function isLivePlayerSource(url = "") {
...
function normalizeRoomDetail(raw = {}, fallback = {}) {
  var _a, _b;
  const detail = raw.live_detail || raw.detail || raw.room || raw;
  const replayVideos = Array.isArray(detail.replayVideos) ? detail.replayVideos : Array.isArray(detail.replays) ? detail.replays : Array.isArray(detail.replayVideoList) ? detail.replayVideoList : Array.isArray(detail.videoList) ? detail.videoList : Array.isArray(detail.videos) ? detail.videos : [];
  return {
    ...detail,
    ...
    notice: detail.notice || ((_a = raw.live_notice) == null ? void 0 : _a.content) || ((_b = raw.live_notice) == null ? void 0 : _b.title) || ""
  };
}
exports.buildBroadcastEntryUrl = buildBroadcastEntryUrl;
exports.getBestLiveUrl = getBestLiveUrl;
exports.getBestReplayUrl = getBestReplayUrl;
exports.getMiniProgramLiveCandidates = getMiniProgramLiveCandidates;
exports.isLivePlayerSource = isLivePlayerSource;
exports.isReplayEntry = isReplayEntry;
exports.normalizeLiveRouteOptions = normalizeLiveRouteOptions;
exports.normalizeRoomDetail = normalizeRoomDetail;
