...
function parseScene(scene = "") {
  ...
  if (!queryTexts.length)
    pushQueryText(decoded);
  queryTexts.forEach((queryText) => {
    queryText.replace(/^\?/, "").split("&").forEach((part) => {
      if (!part)
        return;
...
function normalizeLiveRouteOptions(query = {}) {
  ...
  const isReplay = String(replay || "") === "1" || normalizedMode === "replay" || normalizedLiveType === "replay";
  return {
    ...merged,
    roomCode,
    shareCode,
...
function buildBroadcastEntryUrl(input = {}) {
  ...
  if (options.shareCode && options.shareCode !== options.roomCode)
    params.push(`shareCode=${encodeURIComponent(options.shareCode)}`);
  if (options.liveId)
  ...
  if (options.liveType)
    params.push(`liveType=${encodeURIComponent(options.liveType)}`);
  if (options.tenantId)
    params.push(`tenantId=${encodeURIComponent(options.tenantId)}`);
  if (options.bindId)
    params.push(`bindId=${encodeURIComponent(options.bindId)}`);
  if (options.liveName)
    params.push(`liveName=${encodeURIComponent(options.liveName)}`);
  if (options.cover)
    params.push(`cover=${encodeURIComponent(options.cover)}`);
  if (options.liveCover)
    params.push(`liveCover=${encodeURIComponent(options.liveCover)}`);
  if (options._tc)
    params.push(`_tc=${encodeURIComponent(options._tc)}`);
  if (options.wx_token)
    params.push(`wx_token=${encodeURIComponent(options.wx_token)}`);
...
function isFlvSourceHint(value = "", path = "") {
  return path.endsWith(".flv") || path.includes(".flv/") || path.includes("/flv") || path.includes("httpflv") || path.includes("http-flv") || hasQueryHint(value, ["format", "type", "protocol", "play_type", "stream_type", "source_type", "media_type", "url_type", "ext", "hdl", "source"], ["flv"]);
}
function isHlsSourceHint(value = "", path = "") {
  return path.endsWith(".m3u8") || path.includes(".m3u8/") || path.includes("/hls/") || path.includes("/m3u8") || path.includes("httphls") || path.includes("http-hls") || hasQueryHint(value, ["format", "type", "protocol", "play_type", "stream_type", "source_type", "media_type", "url_type", "ext", "source"], ["hls", "m3u8"]);
}
function isMp4SourceHint(value = "", path = "") {
  return path.endsWith(".mp4") || path.includes(".mp4/") || hasQueryHint(value, ["format", "type", "protocol", "play_type", "stream_type", "source_type", "media_type", "url_type", "ext", "source"], ["mp4"]);
}
function classifySourceToken(token = "") {
  ...
  if (value.includes("live-player") || value.includes("liveplayer"))
    return { type: "live", component: "live-player", rank: 20 };
  return null;
}
function inferSourceMeta(source = {}) {
  ...
  SOURCE_TYPE_FIELDS.forEach((field) => {
    if (source[field] !== void 0 && source[field] !== null)
      tokens.push(source[field]);
  });
