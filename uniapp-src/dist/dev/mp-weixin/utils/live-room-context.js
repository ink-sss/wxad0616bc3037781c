...
function normalizeLiveRoomContext(ctx = {}) {
  ...
  const explicitlyLive = raw.isReplay === false || replay === "0" || normalizedLiveType === "live" || normalizedMode === "live";
  const isReplay = explicitlyLive ? false : raw.isReplay === true || replay === "1" || normalizedLiveType === "replay" || normalizedMode === "replay" || !!replayVideoId;
  return {
    ...raw,
    roomCode,
    room_code: roomCode,
...
function mergeLiveRoomContext(...sources) {
  const merged = {};
...
function buildLiveRoomQuery(ctx = {}, options = {}) {
  ...
  if (normalized.roomId && normalized.roomId !== normalized.liveId)
    appendParam(params, "roomId", normalized.roomId);
  appendParam(params, "tenantId", normalized.tenantId);
  if (normalized.shareCode && normalized.shareCode !== normalized.roomCode)
    appendParam(params, "shareCode", normalized.shareCode);
  ...
  const query = params.filter(([key]) => !existingKeys.has(key)).map(([key, value]) => `${encodeURIComponent(key)}=${encodeURIComponent(value)}`).join("&");
  if (!query)
    return "";
  return `${options.prefix === void 0 ? "?" : options.prefix}${query}`;
}
function appendLiveRoomQuery(url = "", ctx = {}) {
...
exports.appendLiveRoomQuery = appendLiveRoomQuery;
exports.loadLiveRoomContext = loadLiveRoomContext;
exports.mergeLiveRoomContext = mergeLiveRoomContext;
