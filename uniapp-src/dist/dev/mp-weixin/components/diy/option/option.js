...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return {
    a: common_vendor.f($options.dataList, (item, index, i0) => {
      return {
        a: common_vendor.t(item.name || item.title || item.text),
        b: index,
        c: common_vendor.o(($event) => _ctx.$emit("setIndex", index, item.category_id || item.id || ""), index)
      };
    }),
    b: common_vendor.s($options.boxStyle)
  };
}
const Component = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-57f41905"]]);
