"use strict";
Object.defineProperty(exports, Symbol.toStringTag, { value: "Module" });
const common_vendor = require("./common/vendor.js");
const platform_weixin_update = require("./platform/weixin/update.js");
const store_index = require("./store/index.js");
const utils_install = require("./utils/install.js");
if (!Math) {
  "./pages/index/index.js";
  "./pages/product/category.js";
  "./pages/login/login.js";
  "./pages/login/mplogin.js";
  "./pages/login/bindmobile.js";
  "./pages/cart/cart.js";
  "./pages/user/userinfo/userinfo.js";
  "./pages/product/list/list.js";
  "./pages/user/address/address.js";
  "./pages/user/index/index.js";
  "./pages/broadcast/entry.js";
  "./pages/broadcast/replay.js";
  "./pages/prize-record/index.js";
  "./pages/invitation-record/index.js";
  "./pages/invitation/index.js";
  "./pages/report/index.js";
  "./pages/report/report-home.js";
  "./pages/report/report-type.js";
  "./pages/report/report-form.js";
  "./pages/report/report-success.js";
  "./pages/report/report-list.js";
  "./pages/report/report-detail.js";
  "./pages/address/index.js";
  "./pages/address/list.js";
  "./pages/address/edit.js";
  "./pages/address/wxPick.js";
  "./pages/center/index.js";
  "./pages/user/my-wallet/my-wallet.js";
  "./pages/user/my-wallet/my-balance.js";
  "./pages/user/address/add/add.js";
  "./pages/product/search/search.js";
  "./pages/user/address/edit/edit.js";
  "./components/upload/upload.js";
  "./pages/coupon/coupon.js";
  "./pages/user/my-coupon/my-coupon.js";
  "./pages/user/points/points.js";
  "./pages/store/address/address.js";
  "./pages/store/clerkorder.js";
  "./pages/diy-page/diy-page.js";
  "./pages/article/list/list.js";
  "./pages/article/detail/detail.js";
  "./pages/user/my-bargain/my-bargain.js";
  "./pages/store/detail/detail.js";
  "./pages/login/weblogin.js";
  "./pages/login/anchorlogin.js";
  "./pages/user/set/set.js";
  "./pages/shop/shop_list.js";
  "./pages/user/my_attention/my_attention.js";
  "./pages/user/my_collect/my_collect.js";
  "./pages/shop/shop.js";
  "./pages/shop/application_status.js";
  "./pages/user/modify-phone/modify-phone.js";
  "./pages/login/openlogin.js";
  "./pages/coupon/detail.js";
  "./pages/privacy/privacy.js";
  "./pages/webview/webview.js";
  "./pages/user/cash/apply.js";
  "./pages/user/cash/list.js";
  "./pages/shop/index.js";
  "./pages/user/evaluate/list.js";
  "./pages/performance/index.js";
  "./pages/shop/shop_detail.js";
  "./pages/webview/ue.js";
  "./pages/user/my-coupon/share.js";
  "./pages/user/myStoreCoupon/myStoreCoupon.js";
  "./pages/live-push/live-push.js";
  "./pages/live-push/live-push-trtc.js";
  "./pages/live-push/live-list.js";
  "./pages/live-push/liveList/live-detail.js";
  "./pages/live-management/login.js";
  "./pages/live-management/index.js";
  "./pages/live-management/streamers.js";
  "./pages/live-management/members/index.js";
  "./pages/live-management/add-admin/index.js";
  "./pages/live-management/add-admin/edit-user.js";
  "./pages/live-management/add-admin/role-index.js";
  "./pages/live-management/add-admin/role-edit.js";
  "./pages/live-management/risk-members/index.js";
  "./pages/live-management/pro_management/pro_management.js";
  "./pages/live-management/pro_management/pro_add_edit.js";
  "./pages/live-management/live-room/live-room.js";
  "./pages/live-management/order-list/order-list.js";
  "./pages/live-management/order-list/order-detail.js";
  "./pages/live/live-vertical.js";
  "./pages/live/live-horizontal.js";
  "./pages/live/block.js";
  "./pages/live/secret.js";
  "./pages/live/commponents/sign-in.js";
  "./pages/live/live-ks.js";
  "./pages/branch/login.js";
  "./pages/branch/index.js";
  "./pages/branch/scanCode.js";
  "./pages/branch/storeLive.js";
  "./pages/branch/member.js";
  "./pages/branch/writeOff.js";
  "./pages/branch/orderManage.js";
  "./pages/branch/order-detail.js";
  "./pages/branch/rankList.js";
  "./pages/branch/lookLive.js";
  "./pages/branch/agentOrder.js";
  "./pages/branch/apply.js";
  "./pages/branch/cashList.js";
  "./pages/branch/couponList.js";
  "./pages/branch/claimList.js";
  "./pages/branch/memberRoomlist.js";
  "./pages/branch/scanWrittenCode.js";
  "./pages/branch/welfareVoucher.js";
  "./pages/branch/pointDetail.js";
  "./pages/branch/moneyDetail.js";
  "./pages/branch/placeOrder.js";
  "./pages/branch/productWriteoff.js";
  "./pages/branch/productSale.js";
  "./pages/branch/consumptionRank.js";
  "./pages/branch/addManager.js";
  "./pages/branch/staffManage.js";
  "./pages/product/detail/detail.js";
  "./pages/product/detail/look-evaluate/look-evaluate.js";
  "./pages/user/my_shop/my_shop.js";
  "./pages/user/my_shop/my_shop_order.js";
  "./pages/user/my_shop/after_sale.js";
  "./pages/user/my_shop/pro_admin.js";
  "./pages/user/my_shop/my_shop_data.js";
  "./pages/user/my_shop/my_shop_detail.js";
  "./pages/user/my_shop/my_live/notice/notice.js";
  "./pages/user/my_shop/my_live/add/add.js";
  "./pages/user/my_shop/my_live/my-live/edit.js";
  "./pages/user/my_shop/my_live/live/live.js";
  "./pages/user/my_shop/my_live/my-live/record.js";
  "./pages/user/my_shop/my_live/my-live/my-live.js";
  "./pages/user/my_shop/my_live/live-order/live-order.js";
  "./pages/user/my_shop/addProduct.js";
  "./pages/order/confirm.js";
  "./pages/order/pay.js";
  "./pages/order/receipt.js";
  "./pages/order/refund.js";
  "./pages/order/refund-list.js";
  "./pages/order/refund-detail.js";
  "./pages/order/confirm-order.js";
  "./pages/order/recharge.js";
  "./pages/order/evaluate/evaluate.js";
  "./pages/order/express/express.js";
  "./pages/order/refund/apply/apply.js";
  "./pages/order/refund/index/index.js";
  "./pages/order/refund/detail/detail.js";
  "./pages/order/pay-success/pay-success.js";
  "./pages/order/pay-h5/pay-h5.js";
  "./pages/order/deposit-pay.js";
  "./pages/order/alipay-h5/alipay-h5.js";
  "./pages/order/cashier.js";
  "./pages/order/assemble-order.js";
  "./pages/order/express/lottery.js";
  "./pages/order/list.js";
  "./pages/order/detail.js";
  "./pages/order/myorder.js";
  "./pages/order/order-detail.js";
  "./pages/agent/apply/license.js";
  "./pages/agent/order/order.js";
  "./pages/agent/team/team.js";
  "./pages/agent/qrcode/qrcode.js";
  "./pages/agent/index/index.js";
  "./pages/agent/apply/productList.js";
  "./pages/agent/index/store.js";
  "./pages/agent/apply/apply.js";
  "./pages/agent/apply/newApply.js";
  "./pages/agent/index/grade.js";
  "./pages/agent/cash/apply/apply.js";
  "./pages/agent/cash/list/list.js";
  "./pages/agent/shareLive/shareLive.js";
  "./pages/agent/shareLive/live-detail.js";
  "./pagesPlus/chat/chat.js";
  "./pagesPlus/chat/chat_list.js";
  "./pagesPlus/seckill/list/list.js";
  "./pagesPlus/seckill/detail/detail.js";
  "./pagesPlus/assemble/list/list.js";
  "./pagesPlus/assemble/detail/detail.js";
  "./pagesPlus/assemble/fight-group-detail/fight-group-detail.js";
  "./pagesPlus/bargain/list/list.js";
  "./pagesPlus/bargain/detail/detail.js";
  "./pagesPlus/bargain/haggle/haggle.js";
  "./pagesPlus/signin/rule/rule.js";
  "./pagesPlus/signin/signin.js";
  "./pagesPlus/signin/record.js";
  "./pagesPlus/points/list/list.js";
  "./pagesPlus/points/detail/detail.js";
  "./pagesPlus/chat/supplier_chat_list.js";
  "./pagesPlus/chat/supplier_chat.js";
  "./pagesPlus/lottery/lottery.js";
  "./pagesPlus/table/table.js";
  "./pagesPlus/presale/list.js";
  "./pagesPlus/task/index.js";
  "./pagesPlus/lottery/WinningRecord.js";
  "./pagesPlus/lottery/receive.js";
  "./pagesPlus/preview/list.js";
  "./pagesPlus/chat/notice_list.js";
  "./pagesPlus/chat/notice.js";
  "./pagesPlus/card/index.js";
}
const _sfc_main = {
  globalData: {
    migrationStatus: "scaffold"
  },
  onLaunch(options) {
    this.globalData.launchOptions = options || {};
    const updateManager = platform_weixin_update.bindUpdateManager({
      onUpdateReady: () => {
        common_vendor.index.showModal({
          title: "更新提示",
          content: "新版本已经准备好，是否重启应用？",
          success: (res) => {
            if (res.confirm) {
              platform_weixin_update.applyUpdate(updateManager);
            }
          }
        });
      }
    });
  },
  onHide() {
  }
};
function createApp() {
  const app = common_vendor.createSSRApp(_sfc_main);
  const pinia = common_vendor.createPinia();
  const store = store_index.installStore(app);
  app.use(pinia);
  utils_install.installSharedRuntime(app, { store });
  app.config.globalProperties.$migration = {
    source: "compiled-mp-weixin",
    target: "uni-app-vue3-js"
  };
  return {
    app
  };
}
createApp().app.mount("#app");
exports.createApp = createApp;
