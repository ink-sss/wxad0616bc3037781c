...
const STATUS_POLL_TICK_MS = 15e3;
const STATUS_POLL_NORMAL_MS = 15e3;
const STATUS_POLL_SLOW_MS = 6e4;
const STATUS_POLL_FIRST_BACKOFF_MS = 3e4;
const TRUSTED_STATUS_PUSH_TTL_MS = 3e4;
function useLiveHeartbeatStatus(ctx) {
  const {
    liveId,
    sessionId,
    getEnterTimestamp,
  ...
  function startHeartbeat() {
    ...
    heartbeatTimer = setInterval(() => {
      ...
      if (!liveId.value || !sessionId.value || !enterTimestamp)
        return;
      if (getIsPageVisible() === false || getIsPlaybackPaused())
        return;
      if (pushStatus.value === 2 && !isPlaying.value)
        return;
  ...
  function isPushStatusTrusted(now = Date.now()) {
    ...
    if (lastPushAt <= 0 || now - lastPushAt > TRUSTED_STATUS_PUSH_TTL_MS)
      return false;
    const channelState = String((getPushChannelState == null ? void 0 : getPushChannelState()) || "").toLowerCase();
    return !channelState || channelState === "open" || channelState === "connected";
  }
  function getStatusPollInterval(now = Date.now()) {
    const isInactive = getIsPageVisible() === false || getIsMiniWindowActive() || getIsPlaybackPaused();
    const baseInterval = isInactive || isPushStatusTrusted(now) ? STATUS_POLL_SLOW_MS : STATUS_POLL_NORMAL_MS;
    if (consecutivePollFailures <= 0)
      return baseInterval;
    const failureInterval = consecutivePollFailures === 1 ? STATUS_POLL_FIRST_BACKOFF_MS : STATUS_POLL_SLOW_MS;
    return Math.max(baseInterval, failureInterval);
  }
  async function pollLiveStatus(options = {}) {
    if (!liveId.value || typeof getLiveStatus !== "function")
      return;
    const now = Date.now();
    const force = !!options.force;
    if (!force && now - lastStatusPollAt < getStatusPollInterval(now))
    ...
    statusPollPromise = (async () => {
      try {
        ...
        consecutivePollFailures = 0;
        pages_broadcast_utils_liveStatusSnapshot.applyLiveStatusSnapshot({
          pushStatus,
          isPlaying,
          setViewerCountDisplay,
          viewerCount,
...
exports.useLiveHeartbeatStatus = useLiveHeartbeatStatus;
