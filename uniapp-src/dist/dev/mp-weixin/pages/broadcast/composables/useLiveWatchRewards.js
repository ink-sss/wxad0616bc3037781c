...
const _soldOutActivityIds = /* @__PURE__ */ new Set();
...
function normalizeClaimSuccessPayload(raw = {}, fallbackTask = {}) {
  ...
  const explicitClaimStatus = firstValue(base, "claimStatus", "claim_status") ?? firstValue(record, "claimStatus", "claim_status") ?? firstValue(reward, "claimStatus", "claim_status") ?? firstValue(activity, "claimStatus", "claim_status");
...
function hasWatchRewardMarker(payload = {}) {
  ...
  return eventName.includes("watch_reward") || compactName.includes("watchreward") || eventName.includes("watch_duration") || compactName.includes("watchduration") || firstValue(payload, "watchRewardActivityId", "watch_reward_activity_id", "watchRewardId", "watch_reward_id") !== void 0;
}
...
function getClaimKey(myUserId, payload = {}) {
  ...
  const claimedAt = firstValue(payload, "claimedAt", "claimed_at") || firstValue(payload, "createdAt", "created_at") || firstValue(payload, "customerCouponId", "customer_coupon_id") || firstValue(payload, "orderNo", "order_no", "orderId", "order_id") || "done";
...
function readStorageValue(key) {
  var _a;
  try {
    if ((_a = common_vendor.index) == null ? void 0 : _a.getStorageSync) {
...
function writeStorageValue(key, value) {
  var _a, _b;
  try {
    (_b = (_a = common_vendor.index) == null ? void 0 : _a.setStorageSync) == null ? void 0 : _b.call(_a, key, value);
  } catch (e) {
  }
}
...
function writeStoredClaimMarkers(roomCode, customerId, markers) {
  ...
  if (!key)
    return;
  const next = Array.from(new Set(markers)).slice(-100);
...
function normalizeLifecycleAction(payload = {}) {
  ...
  if (["create", "add", "enable", "open", "start", "reopen"].includes(rawAction))
    return "open";
  if (["edit", "update", "modify"].includes(rawAction))
    return "update";
  if (["close", "disable", "stop", "offline"].includes(rawAction))
    return "close";
  if (["delete", "remove"].includes(rawAction))
    return "delete";
  return rawAction;
}
function getClaimErrorMessage(err) {
  var _a;
  return (err == null ? void 0 : err.message) || ((_a = err == null ? void 0 : err.response) == null ? void 0 : _a.msg) || "领取失败，请稍后重试";
}
function isWatchRewardNotReachedError(message) {
  return message.includes("观看时长未达标") || message.includes("观看时长不足") || message.includes("未找到观看记录");
}
function isWatchRewardInvalidError(message) {
  return message.includes("活动未启用") || message.includes("活动已关闭") || message.includes("活动已失效") || message.includes("活动不存在");
}
function isWatchRewardSoldOutError(err, message) {
  var _a;
  const code = toNumber(((_a = err == null ? void 0 : err.response) == null ? void 0 : _a.code) || (err == null ? void 0 : err.code));
  return code === 40003 || message.includes("已领完") || message.includes("领完") || message.includes("已抢完") || message.includes("抢完") || message.includes("已发放完毕") || message.includes("库存不足") || message.includes("名额已满") || message.includes("中奖名额已满");
}
...
function getExplicitStockExhausted(raw = {}) {
  ...
  for (const key of stockKeys) {
    if (raw[key] === void 0 || raw[key] === null || raw[key] === "")
      continue;
...
function readActivityScopedWatchedSec(raw, progress) {
  const hasMarker = Boolean(
    firstValue(progress, "progressStartedAt", "progress_started_at") || firstValue(progress, "activityOpenedAt", "activity_opened_at") || firstValue(raw, "progressStartedAt", "progress_started_at") || firstValue(raw, "activityOpenedAt", "activity_opened_at") || firstValue(raw, "progressSource", "progress_source") === "activity"
  );
  return pickNumber(
    ...
    hasMarker ? firstValue(progress, "effectiveWatchedSec", "effective_watched_sec") : void 0,
    hasMarker ? firstValue(progress, "watchedSec", "watched_sec") : void 0,
    hasMarker ? firstValue(raw, "effectiveWatchedSec", "effective_watched_sec") : void 0,
    hasMarker ? firstValue(raw, "watchedSec", "watched_sec") : void 0
  );
}
function hasActivityScopedProgress(raw, progress, watchedSec) {
  return watchedSec !== null || Boolean(
    firstValue(progress, "progressStartedAt", "progress_started_at") || firstValue(progress, "activityOpenedAt", "activity_opened_at") || firstValue(progress, "activityRemainingSec", "activity_remaining_sec") !== void 0 || firstValue(raw, "progressStartedAt", "progress_started_at") || firstValue(raw, "activityOpenedAt", "activity_opened_at") || firstValue(raw, "activityRemainingSec", "activity_remaining_sec") !== void 0 || firstValue(raw, "progressSource", "progress_source") === "activity"
  );
}
function buildClaimFailureProgressPatch(err, message, currentTask = {}) {
  var _a;
  const data = ((_a = err == null ? void 0 : err.response) == null ? void 0 : _a.data) || (err == null ? void 0 : err.data) || {};
...
function getTaskRecencyKey(task = {}) {
  var _a, _b;
  return String(
    firstValue(task, "activityOpenedAt", "activity_opened_at") || ((_a = task.userProgress) == null ? void 0 : _a.activityOpenedAt) || ((_b = task.user_progress) == null ? void 0 : _b.activity_opened_at) || firstValue(task, "openedAt", "opened_at") || firstValue(task, "updatedAt", "updated_at") || firstValue(task, "createdAt", "created_at") || ""
  );
}
...
function isVisibleWatchRewardTask(task = {}, myUserId = 0) {
  ...
  if (task.claimed || claimStatus === 3)
    return false;
  if (_soldOutActivityIds.has(getActivityId(task)))
    return false;
  return getActivityId(task) > 0 && toNumber(firstValue(task, "activityStatus", "activity_status"), 1) === 1 && (claimStatus === 0 || claimStatus === 1);
}
function normalizeTask(raw, existing, options = {}) {
  ...
  if ((existing == null ? void 0 : existing.claimStatus) === 3 && getClaimMarker(existing) === getClaimMarker(raw) && claimStatus !== 4 && claimStatus !== CLAIM_STATUS_SOLD_OUT) {
    claimStatus = 3;
  }
  if ((existing == null ? void 0 : existing.claimStatus) === 2 && claimStatus !== 3 && claimStatus !== 4 && claimStatus !== CLAIM_STATUS_SOLD_OUT) {
    claimStatus = 2;
  }
  const explicitStockExhausted = getExplicitStockExhausted(raw);
  const stockExhausted = claimStatus === CLAIM_STATUS_SOLD_OUT || explicitStockExhausted === true || explicitStockExhausted === null && Boolean(existing == null ? void 0 : existing.stockExhausted);
  ...
  if (claimStatus !== 2 && claimStatus !== 3 && claimStatus !== 4 && claimStatus !== CLAIM_STATUS_SOLD_OUT) {
  ...
  return {
    ...
    collapsed: (existing == null ? void 0 : existing.collapsed) || false,
    claimedAt: firstValue(raw, "claimedAt", "claimed_at") || (existing == null ? void 0 : existing.claimedAt) || "",
    orderNo: firstValue(raw, "orderNo", "order_no", "orderId", "order_id") || (existing == null ? void 0 : existing.orderNo) || "",
    customerCouponId: firstValue(raw, "customerCouponId", "customer_coupon_id") || (existing == null ? void 0 : existing.customerCouponId) || 0
  };
}
function useLiveWatchRewards(ctx) {
  const {
    roomCode,
    liveId,
    liveTenantId,
  ...
  function getMyNickname() {
    return (resolveMyNickname == null ? void 0 : resolveMyNickname()) || "我";
  }
  const watchRewardTasks = common_vendor.ref([]);
  const showWatchRewardPanel = common_vendor.ref(false);
  const watchRewardPanelOpenKey = common_vendor.ref(0);
  const watchRewardResult = common_vendor.ref(null);
  const watchRewardBroadcast = common_vendor.ref(null);
  const watchRewardBroadcastRenderKey = common_vendor.ref(0);
  const seenClaimKeys = /* @__PURE__ */ new Set();
  const claimingIds = /* @__PURE__ */ new Set();
  const autoClaimCooldownUntil = /* @__PURE__ */ new Map();
  const localProgressWatchedSec = /* @__PURE__ */ new Map();
  const pendingProgressResetIds = /* @__PURE__ */ new Set();
  let _lastSyncAt = 0;
  let tickTimer = null;
  let reloadTimer = null;
  let broadcastTimer = null;
  const visibleWatchRewardTasks = common_vendor.computed(() => {
    return watchRewardTasks.value.filter((task) => isVisibleWatchRewardTask(task, toNumber(myUserId.value)));
  });
  const hasVisibleWatchRewardTasks = common_vendor.computed(() => {
    return visibleWatchRewardTasks.value.length > 0;
  });
  const watchRewardEntryLabel = common_vendor.computed(() => {
    const claimableTask = visibleWatchRewardTasks.value.find((task) => task.claimStatus === 1);
    if (claimableTask)
      return "领取";
    const pendingTask = visibleWatchRewardTasks.value.filter((task) => task.claimStatus === 0 && task.remainingSec > 0).sort((a, b) => a.remainingSec - b.remainingSec)[0];
    if (pendingTask)
      return formatEntryCountdown(pendingTask.remainingSec);
    return "";
  });
  function syncFromLiveDetail(detail) {
    _lastSyncAt = Date.now();
    const marketingConfig = (detail == null ? void 0 : detail.marketingConfig) || (detail == null ? void 0 : detail.marketing_config) || {};
    ...
    watchRewardTasks.value = list.map((item) => {
    ...
    }).filter((task) => isVisibleWatchRewardTask(task, toNumber(myUserId.value))).sort(compareWatchRewardTasksDesc);
  ...
  async function reloadWatchRewards({ minStaleMs = 0 } = {}) {
    if (!roomCode.value || typeof getLiveDetailApi !== "function")
      return;
  ...
  async function claimWatchRewardTask(taskOrId) {
    ...
    if (!activityId || claimingIds.has(activityId) || typeof claimWatchRewardApi !== "function")
      return;
    ...
    try {
      ...
      const result = await claimWatchRewardApi({
        activityId,
        activity_id: activityId,
        roomId: toNumber(liveId == null ? void 0 : liveId.value),
        room_id: toNumber(liveId == null ? void 0 : liveId.value),
        liveId: toNumber(liveId == null ? void 0 : liveId.value),
        live_id: toNumber(liveId == null ? void 0 : liveId.value),
        tenantId: toNumber(liveTenantId == null ? void 0 : liveTenantId.value),
        tenant_id: toNumber(liveTenantId == null ? void 0 : liveTenantId.value),
      ...
      if ((currentTask == null ? void 0 : currentTask.claimType) === 2) {
        autoClaimCooldownUntil.set(activityId, Date.now() + 3e4);
      }
      ...
      if (isWatchRewardInvalidError(msg)) {
        ...
        removeWatchRewardTask(activityId);
        if ((currentTask == null ? void 0 : currentTask.claimType) !== 2) {
          common_vendor.index.showToast({ title: "奖品已经被抢光了", icon: "none" });
        }
        return;
        ...
        updateTask(activityId, { claimStatus: (currentTask == null ? void 0 : currentTask.reached) ? 1 : 0 });
      }
  ...
  function handleWatchRewardWinNotify(message) {
    ...
    if (!isWatchRewardPayload(payload))
      return;
    const customerId = getCustomerId(myUserId, payload);
    if (myUserId.value > 0 && customerId > 0 && customerId !== toNumber(myUserId.value))
      return;
    pages_broadcast_composables_liveLotteryMessage.appendLotteryWinMessage(
      appendSystemMessage,
      handledMessageIds,
      payload,
      {},
  ...
  function handleWatchRewardBroadcast(message) {
    ...
    if (!isWatchRewardPayload(payload))
      return;
    pages_broadcast_composables_liveLotteryMessage.appendLotteryWinMessage(appendSystemMessage, handledMessageIds, payload, {});
    if (broadcastTimer)
  ...
  function advanceLocalProgress() {
    ...
    let changed = false;
    watchRewardTasks.value = watchRewardTasks.value.map((task) => {
      if (task.claimStatus === 3 || task.claimStatus === 4 || task.activityStatus !== 1)
        return task;
  ...
  function checkAutoClaims() {
    watchRewardTasks.value.forEach((task) => {
      if (task.activityStatus === 1 && task.claimType === 2 && task.claimStatus === 1 && task.reached && Date.now() >= (autoClaimCooldownUntil.get(task.activityId) || 0)) {
  ...
  function startTicker() {
    if (tickTimer)
      return;
    tickTimer = setInterval(advanceLocalProgress, 1e3);
  }
  function stopTicker() {
    if (!tickTimer)
      return;
...
exports.useLiveWatchRewards = useLiveWatchRewards;
